...
enum UITestEnvironment {
    ...
    static var isEnabled: Bool {
        boolValue(for: modeKey)
    }

    static var shouldForceMainTab: Bool {
        boolValue(for: forceMainTabKey)
    }

    static var routeBeforeMainTab: Bool {
        boolValue(for: routeBeforeMainTabKey)
    }

    static var deeplinkURLs: [URL] {
        guard
            let raw = ProcessInfo.processInfo.environment[deeplinksJSONKey],
            !raw.isEmpty
        else {
            return []
        }
